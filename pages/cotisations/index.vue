<script setup>
import axios from "axios"
const modalajout = ref(false);
const modaledit = ref(false);
const data = ref([]);
const message = ref('');
const faritra = ref([])
const diosezy = ref([])

const api = "http://localhost:5000";

const datainput = reactive({
    id_categorie : "",
    id_fivondronana : "",
    id_mpikatroka : ""
})

const dataedit = reactive({
    id : null,
    nom_categorie : '',
    tarif : '',
    
})

const getDiosezy = async () => {
    try {
        const response = await axios.get(`${api}/diosezy`);
        diosezy.value = response.data;
    } catch (error) {
        error.value = "Failed to load data";
        console.log(error);
    }
}

const getData = async () => {
    try {
        const response = await axios.get(`${api}/cotisations`);
        data.value = response.data;
        
    } catch (error) {
        error.value = "Failed to load data";
        console.log(error);
    }
}

const InsertCategory = () => {
    try {
        const response = axios.post(`${api}/categorie/create`, {
            nom_categorie : datainput.nom_categorie,
            tarif : datainput.tarif,
        });
        getData();
        message.value = "Insertion effectué avec succès !"
    } catch (error) {
        console.log(error);
    }
}

const UpdateCategorie = () => {
    try {
        const response = axios.put(`${api}/categorie/update/${dataedit.id}`, {
            nom_categorie : dataedit.nom_categorie,
            tarif : dataedit.tarif,
        });

        getData();

        message.value = "Modification effectué avec succès !"
    } catch (error) {
        console.log(error);
    }
}

const Delete = async (id) => {
    if(confirm('Etes-vous sûr de vouloir suppimer cette donnée ?')){
        try {
            axios.delete(`${api}/categorie/delete/${id}`);
            getData();
            alert("Succès !!");
        } catch (error) {
            console.log(error);
        }
    }
}

const OpenEditForm = (item) => {
    dataedit.id = item.id_categorie;
    dataedit.nom_categorie = item.nom_categorie;
    dataedit.tarif = item.tarif;
}

onMounted(() => {
    getData();
    getDiosezy();
})

</script>
<template>
    <div class="p-4 w-full">
        <div>
            <div class="py-2 bg-white px-4 flex justify-between items-center">
                <ul class="flex text-sm text-gray-800 space-x-2">
                    <li><a href="/">Tableau de bord</a></li>
                    <li>></li>
                    <li><a href="/faritra">cotisations</a></li>
                    <li>></li>
                    <li><a href="/faritra">listes</a></li>
                </ul>
                <div>
                    <button @click="modalajout = !modalajout" class="bg-blue-600 hover:bg-blue-500 text-white px-3 py-2 rounded-md">Hampiditra</button>
                </div>
            </div>
        </div>
        <div>
            <div class="w-full mx-auto p-3 shadow-md rounded-md bg-white mt-[50px]">
                <table class="table w-full">
                    <tr class="bg-gray-200 font-semibold">
                        <td class="py-2">
                            ID
                        </td>
                        <td>
                            Anarana
                        </td>
                        <td>
                            Fivondrona
                        </td>
                        <td>
                            Payement
                        </td>
                        <td>
                            Année
                        </td>
                        <td>
                            Action
                        </td>
                    </tr>
                    <tbody>
                        <tr v-for="(cat, key) in data">
                            <td class="py-2">{{ key + 1}}</td>
                            <td>{{ cat.nom_categorie}}</td>
                            <td>{{ cat.tarif }} MGA</td>
                           
                            <td class="flex space-x-3 items-center py-2">
                                <button @click="Delete(cat.id_categorie)" class="px-3 py-1 rounded-md hover:bg-red-500 bg-red-600 text-white text-sm">Supprimer</button>
                                <button @click="OpenEditForm(cat); modaledit = !modaledit" class="px-3 py-1 rounded-md hover:bg-blue-500 bg-blue-600 text-white text-sm">Modifier</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
 <div  v-if="modalajout" class="h-full w-full absolute top-0 bottom-0 left-0 right-0 bg-black/25">
    <div class="w-1/3 mx-auto p-3 shadow-md rounded-md bg-white mt-[50px]">
                <div class="flex justify-between items-center">
                   <p class="font-semibold text-2xl text-center">Ajout Cotisations</p>
                   <button @click="modalajout = !modalajout; message ='';">Fermer</button>
                </div>
                <div class="mt-10">
                  
                    <div v-if="message" class="py-2 text-center bg-green-100 text-green-900">
                        {{ message }}
                    </div>
                    <form @submit.prevent="InsertCategory" class="space-y-2">
                        <div class="flex flex-col">
                            <label for="">Diosezy</label>
                            <select class="px-2 h-12 border border-gray-400 rounded-md">
                                <option v-for="d in diosezy" value="">{{ d.nom_diosezy }}</option>
                            </select>
                        </div>
                        <div class="flex flex-col">
                            <label for="">Faritra</label>
                            <select class="px-2 h-12 border border-gray-400 rounded-md">
                                <option v-for="d in diosezy" :value="d.id">{{ d.nom_diosezy }}</option>
                            </select>
                        </div>
                        
                        <div class="flex flex-col">
                            <label for="">Tarif</label>
                            <input v-model="datainput.tarif" class="px-2 h-12 border border-gray-400 rounded-md" type="text" placeholder="Tarif">
                        </div>
                        <div>
                            <button class="bg-blue-600 hover:bg-blue-500 text-white px-3 py-2 rounded-md" type="submit">Sauvegarder</button>
                        </div>
                    </form>
                </div>
            </div>
 </div>

 <div  v-if="modaledit" class="h-full w-full absolute top-0 bottom-0 left-0 right-0 bg-black/25">
    <div class="w-1/3 mx-auto p-3 shadow-md rounded-md bg-white mt-[50px]">
                <div class="flex justify-between items-center">
                   <p class="font-semibold text-2xl text-center">Modification catégorie</p>
                   <button @click="modaledit = !modaledit; message = ''">Fermer</button>
                </div>
                <div class="mt-10">
                    
                    <div v-if="message" class="py-2 text-center bg-green-100 text-green-900">
                        {{ message }}
                    </div>
                    <form @submit.prevent="UpdateCategorie" class="space-y-2">
                        <div class="flex flex-col">
                            <label for="">Catégorie</label>
                            <input v-model="dataedit.nom_categorie" class="px-2 h-12 border border-gray-400 rounded-md" type="text" placeholder="Catégorie">
                        </div>
                        
                        <div class="flex flex-col">
                            <label for="">Tarif</label>
                            <input v-model="dataedit.tarif" class="px-2 h-12 border border-gray-400 rounded-md" type="text" placeholder="Tarif">
                        </div>
                        <div>
                            <button class="bg-blue-600 hover:bg-blue-500 text-white px-3 py-2 rounded-md" type="submit">Mis à jour</button>
                        </div>
                    </form>
                </div>
            </div>
 </div>
</template> 